class Ability
  ...
  def initialize(user)
    ...
    if (user.role? :admin) || (user.role? :moderator) || (user.role? :editor) || (user.role? :user)
      ...
      can %i[update decor], User, id: user.id
      can %i[read manage list], Event, user: { id: user.id }
    end

    if (user.role? :moderator) || (user.role? :editor) || (user.role? :user)
      can :rebuild, Result
      cannot %i[manage read], [Room, Manufacture, ManufactureGroup, Substrate, Table, Operation]
    ...
    if user.has_group? :lab181
      can %i[manage read], [Manufacture, ManufactureGroup, Operation, ManufactureOperation]
      can :actual, [Manufacture, ManufactureGroup]
